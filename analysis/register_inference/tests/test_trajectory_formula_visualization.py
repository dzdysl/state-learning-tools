...
class TrajectoryFormulaVisualizationTests(unittest.TestCase):
    ...
    def test_offline_filters_formula_viewport_and_determinism(self):
        ...
        for text in ("plotly.js", "轨迹归类算法 B", "二维公式候选", "稳定性推断聚合", "前序最简与重划分", "候选赋值方案", "E0085:S017:L17 的前序事件 E0042", "R3–R10 的 8 次 E0042 事件", "逐轮集合前像 {6,7}", "E0172", "pseudo_hold", "pseudo_reverse_preimage", "real_downlink", "退化：静态三元组", "未确认：伪边界", "不作为稳定公式", "PRE_GLOBAL_RANGE", "window.algorithmBViewer", 'data-testid="assignment-scenario"', "v=>v,false,preferred.assignment", 'id="candidate-type"', 'id="input-output"', 'id="signal"', 'id="eid"', 'id="projection"', 'id="formula-kind"', 'id="candidate-group"', "r' = r + 1", "GLOBAL_RANGES", "scaleanchor:'y'", "legendgroup:group", "groupclick:'togglegroup'", "new Map()", "seenGroups", "formulaKeys", "colorByDisplay", "traceFor(x,color,first)", "showlegend:showLegend", "无公式（退化轨迹）", "常数赋值", "单位仿射式", "简单聚合公式", '"points":[[1,2],[1,4]]', "vertical.x.push(...v.points.map", '"plotly_up_angle":45.0', "stable_aggregation", "visibleTraceItems"):
    ...
    def test_predecessor_repartition_static_boundaries(self):
        semantics = VIS.predecessor_repartition_semantics()
        self.assertIn("候选赋值方案", semantics["assignment"])
        self.assertIn("不生成边级公式", semantics["pseudo_reverse"])
        self.assertIn("不重新拟合公式", semantics["eligible"])
